...
- compiled-entity: [[entities/Hermes|Hermes]] — source 是 Hermes 实体页的主要证据：学习循环、会话血缘、记忆插件、cron、delegate、batch。
- compiled-concept: [[concepts/子代理委派模式|子代理委派模式]] — `delegate_task` 提供隔离上下文、受限工具、只回收摘要的委派证据。
- compiled-concept: [[concepts/会话分层存储与压缩|会话分层存储与压缩]] — SQLite+FTS5 会话血缘和压缩续篇恢复是分层会话存储的实现证据。
- updates: [[concepts/Agent Runtime|Agent Runtime]] — 作为学习型/长期运行取向补入 runtime 变体分类轴。
- updates: [[concepts/Agent 记忆|Agent 记忆]] — 补充记忆 provider 插件和会话血缘机制。
- updates: [[synthesis/Agent 系统工程综述|Agent 系统工程综述]] — 进入运行时变体对比表的学习型一行。
- contrasts-with: [[entities/OpenClaw|OpenClaw]] — raw 扩展阅读直接对比 Hermes 与 OpenClaw 记忆系统差异。
