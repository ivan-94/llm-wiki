...
- compiled-concept: [[concepts/低代码 Agent 平台 vs 代码优先 Runtime|低代码 Agent 平台 vs 代码优先 Runtime]] — source 提供代码优先一侧（LangChain/LangGraph/DeepAgents 三层分层）的核心证据。
- compiled-concept: [[concepts/Workflow 与 Agent 的边界|Workflow 与 Agent 的边界]] — LangGraph 同时支持 workflow 编排和 agent 动态决策，是边界非二选一的证据。
- updates: [[concepts/Agent Runtime|Agent Runtime]] — DeepAgents 作为成品 runtime、LangGraph 作为执行运行时补入 runtime 分层。
