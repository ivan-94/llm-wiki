...
- compiled-concept candidate: [[concepts/Agent Harness|Agent Harness]] — 可提炼 harness 的定义、组件、失败闭环和运行时心智模型。
- compiled-concept candidate: [[concepts/Agent Runtime|Agent Runtime]] — source 将 harness 类比为 runtime + compiler，可支撑 runtime 边界定义。
- map-entry candidate: [[maps/Vibe Coding 学习地图|Vibe Coding 学习地图]] — 可作为 AI coding 工作系统设计主题的核心入口。
- compiled-synthesis candidate: [[synthesis/Agent Harness 工程化框架|Agent Harness 工程化框架]] — 可与同目录 Harness 循环、Skills 图形成跨 source 综合。
