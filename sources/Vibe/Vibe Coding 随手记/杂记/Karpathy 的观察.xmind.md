...
- compiled-concept: [[concepts/Agent Coding Guardrails|Agent Coding Guardrails]] — partial source；本 source 提供澄清、简洁、精准修改三类 guardrails 的原始观察（导出末尾未完成）。
- compiled-entity: [[entities/Andrej Karpathy|Andrej Karpathy]] — 根节点外链指向 Karpathy 对模型写代码问题的观察帖。
- compiled-synthesis: [[synthesis/Vibe Coding 工程化深度模型|Vibe Coding 工程化深度模型]] — guardrails 对应 L3 决策控制与 L4 证据层的约束材料。
...
- status 保持 `partial`：末尾分支未完成，不宜单独支撑完整 guardrails 页面，仅作补充证据。
