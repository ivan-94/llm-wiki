...
- compiled-entity: [[entities/Codex|Codex]] — Goal Mode 是 Codex 的核心能力，本 source 提供量化目标/反馈/外部记忆三要素。
- related: [[concepts/Agent 反馈回路|Agent 反馈回路]] — 可提炼目标、评分、停止条件和持续迭代循环。
- compiled-concept: [[concepts/反馈工程（Feedback Engineering）|反馈工程（Feedback Engineering）]] — Goal Mode 是反馈工程的典型实现案例。
- related: [[concepts/Agent 全局记忆|Agent 全局记忆]] — 可沉淀 PLAN.md、EXPERIMENTS.md、EXPERIMENT_NOTES.md 的角色分工。
- contains: [[sources/Vibe/工具/codex/cli.png|cli.png]] — Codex CLI 的操作层，两者共同覆盖 Codex 产品全貌。
- map-entry: [[maps/Vibe Coding 工具地图|Vibe Coding 工具地图]] — Codex 工具簇的 Goal 模式来源。
