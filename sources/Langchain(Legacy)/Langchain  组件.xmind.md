...
- related: [[sources/Agent/LangChain.xmind|Agent/LangChain.xmind]] — 现有 source 与本 source 同属 LangChain 总览和应用编排证据（legacy · not verified）。
- related: [[concepts/提示语工程|提示语工程]] — Model I/O 中的 PromptTemplate、ChatPromptTemplate、few-shot 和 parser 细节可补充提示语工程实践（legacy · not verified）。
- related: [[concepts/上下文工程|上下文工程]] — Retrieval、Memory、Callbacks 和 Composition 都服务于上下文构造、传递和观测（legacy · not verified）。
- compiled-entity: [[entities/LangChain|LangChain]] — source 提供 LangChain 组件分层（Model I/O/Retrieval/Callbacks/Memory/Composition）的实体证据（legacy · not verified）。
- related: [[sources/Langchain(Legacy)/Langchain 案例学习.xmind|Langchain 案例学习.xmind]] — 案例 source 与组件 source 形成"设计模式 ↔ 实现组件"对照关系（legacy · not verified）.
