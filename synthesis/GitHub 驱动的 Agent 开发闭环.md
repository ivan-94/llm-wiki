...
HAT 是 PR 合并前的用户路径证据门，不应只是一句“人工验收通过”。闭环里最稳的分层是：

- `hat-prepare`：把环境、账号、数据、schema/version、P0/P1/P2 清单和幂等 `prepare.sh` 写成可交接产物。
- `hat-run`：只消费 prepare 产物，使用 browser/Playwright/Codex 浏览器等工具执行，P0 失败优先停止并保存 summary、logs、截图和人工介入项。
- `hat-dispatch`：从 `hat-ready` PR 批量分发验收，但必须判断环境隔离和数据冲突，不能默认并发。
- HAT-friendly 项目改造：前端提供稳定选择器和 URL 化状态，后端提供日志、OTEL console 和 Request Id，降低 Agent 验收探索成本。
...
- [[sources/Vibe/工具/mattpocock:skills  ⭐/Ext HAT - Hand Acceptance Test Prepare验收测试准备hat-prepare.xmind|hat-prepare]] — HAT gate 的准备协议。
- [[sources/Vibe/工具/mattpocock:skills  ⭐/Ext HAT - Hand Acceptance Test Run验收测试执行hat-run.xmind|hat-run]] — HAT gate 的执行协议和证据输出。
- [[sources/Vibe/工具/mattpocock:skills  ⭐/Ext HAT workerHAT 批量执行器hat-dispatch.xmind|hat-dispatch]] — HAT gate 的批量调度限制。
...
- related: [[concepts/HAT（Hand Acceptance Test）|HAT（Hand Acceptance Test）]] — HAT 是闭环中 PR 合并前的用户路径证据门
- related: [[concepts/Agent 上下文审计|Agent 上下文审计]] — 上下文审计可检验项目入口是否足够支持下游 Agent 执行闭环
