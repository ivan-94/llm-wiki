...
- 产品方法论补充：human podcast source 强调为未来模型能力构建、从 latent demand 产品化 Claude.md/plan mode/verbose mode/co-work，并把 subagents 理解为未互相污染的上下文窗口。
- 动态工作流补充：适合长时间运行、大规模并行和对抗性验证；通过 JavaScript 脚本持有循环、分支和中间结果，并支持中断恢复。
- Skills 实践补充：Anthropic blog source 把 Claude Code skills 描述为文件夹式上下文包，可包含 scripts、references、assets、data、hooks 和 memory；内部 marketplace 通过 sandbox traction 晋升。
...
- [[sources/Vibe/工具/Claude Code/动态工作流.xmind|动态工作流]]
- [[sources/Vibe/Vibe Coding 随手记/大规模 AI Coding/claude code 大型项目落地.PNG|claude code 大型项目落地]]
- [[human/sources/inbox/cook-podcast/2026-05-31_Y Combinator Startup Podcast_Inside Claude Code With Its Creator Boris Cherny|Inside Claude Code With Its Creator Boris Cherny]] — human source，补充 Claude Code 创造者访谈中的产品和 agent workflow 方法论。
- [[human/sources/inbox/cook-blog/2026-06-05_Claude_Code团队如何使用技能_Anthropic|Claude Code 团队如何使用技能]] — human source，补充 Claude Code skills 的内部分类、gotchas、hooks、memory 和 marketplace 分发。
