...
Vibe Coding 的典型工作模式分两阶段：

**Phase 1：粗生成（0→60 分）**
- 用自然语言描述意图，让 Agent 快速生成结构完整的初版代码。
- 价值：快速验证方向，快速看到可运行结果，用于原型探索或需求澄清。
- 典型风险：表面合理但语义、集成和长期工程质量不可靠。

**Phase 2：精修（60→90+ 分）**
- 通过 spec 精化、决策控制、反捷径测试、代码审查和 HAT 把质量推进到可交付。
- 小任务可在此阶段快速收尾；大任务需要进入多层控制系统。

**原型不可产品化**：Vibe Coding 做的原型探索，其代码不应直接演化为生产代码，否则技术负债从原型期就埋下。产品方向收敛后，从 spec 出发重新实现。

## Vibe vs 严肃 Coding 阶段分工

| 场景 | 适合 Vibe Coding | 需要转向严肃工程 |
| --- | --- | --- |
| 简单任务 | oneshot 直接实现 | — |
| 需求验证 | 原型探索 + 快速迭代 | 原型收敛后重新实现 |
| 业务项目 | 借助规格控制漂移 | 涉及权限/安全/数据时强治理 |
| 严肃系统 | 仅用于粗稿生成 | 完整 Agentic Engineering 流程 |
