...
- human source 补充：输出质量门至少需要 test cases、metrics、threshold；没有 gate 时，prompt、memory 和 context file 只是输入侧修补。
- human source 补充：评估也可用于长期加载的 skill 文本，SkillOpt 式流程用 train/val/test、answer key 和 baseline 决定是否接受 skill 编辑。
...
- explicit: [[human/sources/inbox/cook-tweet/2026-06-01_AI输出质量控制与Hermes评估循环_EXM7777|AI 输出质量控制与 Hermes 评估循环]] — human source，补充 eval loop、benchmark 三要素和失败样本回写。
- explicit: [[human/sources/inbox/cook-tweet/2026-06-03_用SkillOpt训练可进化Agent技能_SkillOpt|用 SkillOpt 训练可进化 Agent 技能]] — human source，补充 skill 文本评估与 answer key/held-out data 门控。
