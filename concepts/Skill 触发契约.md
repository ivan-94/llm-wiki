...
- skill 审查应检查触发质量、边界、行为契约、冲突优先级、验证方式和输出可用性，而不只是润色描述。
- DSL-backed skill 可把 `activate_when`、inputs、outputs、安全边界和审批结构化成 contract，让触发契约从散文约定变成可检查形状。
- Anthropic Claude Code source 补充：description 是给模型做路由判断的接口，gotchas、setup、progressive disclosure 和 on-demand hooks 都是在触发后继续塑形 Agent 行为。
- `last30days-skill` 补充：复杂触发契约还包括 query quality、`--plan` JSON、source availability、degraded warning、footer pass-through 和最终 voice law。
- `brooks-lint` 补充：触发后契约要约束扫描顺序、风险词表、输出字段、严重度和 clean-case eval，防止 review skill 被正确触发后仍输出 slop。
...
- [[sources/Vibe/工具/mattpocock:skills  ⭐/Ext Skills 审查.png|Ext Skills 审查]] — 补充触发质量、边界、冲突、验证和输出可用性等审查维度。
- [[sources/Vibe/工具/mattpocock:skills  ⭐/create-dsl-skills.png|create-dsl-skills.png]] — 补充 `activate_when`、inputs、outputs 和安全边界前置的 DSL 契约表达。
- [[human/sources/inbox/cook-blog/2026-06-05_Claude_Code团队如何使用技能_Anthropic|Claude Code 团队如何使用技能]] — human source，补充 description、gotchas、progressive disclosure 和 hooks 的触发语义。
- [[human/sources/inbox/cook-github/2026-06-09_近30天多源研究技能_mvanhorn_last30days-skill|近30天多源研究技能]] — human source，补充超长 skill contract、query planning 和输出契约案例。
- [[human/sources/inbox/cook-github/2026-06-10_AI审查Slop治理_brooks-lint_hyhmrright_brooks-lint|brooks-lint：AI 审查 Slop 治理]] — human source，补充触发后扫描步骤、报告模板、eval 和 validator。

## Relations

- part-of: [[concepts/Agent Skills|Agent Skills]] — 触发契约是 skill 设计的核心控制面，决定 skill 能否有效工作。
- strengthened-by: [[concepts/Skill Contract DSL|Skill Contract DSL]] — DSL 可以把触发条件、输入、输出和安全边界结构化。
- related: [[concepts/上下文工程|上下文工程]] — 强/弱注入是上下文工程在 skill 层的体现。
- related: [[concepts/Agent 上下文审计|Agent 上下文审计]] — 上下文审计可用真实 sub-agent 行为检验触发契约是否清晰。
