...
SbE 在 AI Coding 工作流中的五个质量门（来源于 `ai_coding.png` 验收链路）：

1. **业务语言一致**：需求、spec、示例使用同一个领域术语（不是代码词）。
2. **示例无歧义**：每个 example 只对应一个正确行为解释，不接受"看情况"。
3. **反例覆盖**：至少有一个反例（not happy path）被写成可执行 example。
4. **测试可追溯**：每个关键 example 都能对应到一条可执行的验收测试。
5. **活文档更新**：当业务规则改变时，示例和测试必须同步更新（活文档）。
