...
description: 将 X/Twitter 单条 post、thread 或 X article 链接烹饪成 AI wiki human inbox 笔记。用户给出 x.com/twitter.com 的推文、thread 或文章链接，并希望用 browser-use 打开页面、消费可见内容、生成中文消化笔记和信息图，而不是 ingest 进 canonical wiki graph 时使用。
---
...
- 只能用 browser-use 打开和消费 X 页面内容。不要使用 X API、第三方镜像、HTTP 抓取、搜索引擎缓存、Chrome 登录态 fallback、cookie、token 或用户账号密码。
...
- 最终笔记直接 embed 原始 X 链接，不保存完整原文，也不逐条复制 thread 原文。
...
1. 用 browser-use 打开输入 URL，等待页面稳定。
...
3. 生成信息图：
   ...
   - imagegen 原图保存到 cache，再把最终选定图移动或转换为 `human/inbox/cook-tweet/assets/<note-stem>/infographic.webp`。
...
<blockquote class="twitter-tweet">
  <a href="<canonical-or-input-url>"></a>
</blockquote>

<script async src="https://platform.twitter.com/widgets.js" charset="utf-8"></script>

如果当前 Obsidian 环境不渲染 X embed，请打开：<canonical-or-input-url>
...
- `原文` 只放 X embed 和可打开 URL，不粘贴完整原文。
...
- canonical URL。
- source_kind。
- capture method：`browser-use only`。
