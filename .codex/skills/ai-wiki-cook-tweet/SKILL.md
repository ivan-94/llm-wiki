...
description: 将 X/Twitter 单条 post、thread 或 X article 链接烹饪成 AI wiki human inbox 笔记。用户给出 x.com/twitter.com 的推文、thread 或文章链接，并希望用未登录可见页面浏览器自动化（browser-use/agent-browser）打开页面、消费可见内容、生成中文消化笔记和信息图，而不是 ingest 进 canonical wiki graph 时使用。
---
...
.codex/cache/cook-tweet/<status-id-or-url-hash>/
  capture.md
  screenshot.png          # 可选，记录可见页面浏览器自动化当时看到的页面
...
- 只能用未登录可见页面浏览器自动化打开和消费 X 页面内容；当前可用实现通常是 browser-use 或 agent-browser。不要使用 X API、第三方镜像、HTTP 抓取、搜索引擎缓存、Chrome 登录态 fallback、cookie、token 或用户账号密码。
- 如果可见页面里看不到足够正文，必须按本文状态机 fail fast。不要根据 URL 标题、搜索片段、记忆或猜测生成 cooked note。
...
- 最终笔记按 Obsidian 官方网页嵌入规则直接 embed tweet 或保留可打开原文链接，不保存完整原文，也不逐条复制 thread 原文。能解析 status URL 时使用 Markdown 外部图片语法：`![](https://twitter.com/<handle>/status/<id>)`，同时保留 `x.com` 原链接作为 fallback；只有 article URL 且无法确认 status URL 时，不伪造 tweet embed。
...
1. 用未登录可见页面浏览器自动化打开输入 URL，等待页面稳定。
2. 归一化 canonical URL，优先保留 `https://x.com/<handle>/status/<id>`。如果只能看到 `twitter.com`，保留原 URL 并记录。
   - 如果能解析 `<handle>` 和 `<status-id>`，同时生成 `embed_url: https://twitter.com/<handle>/status/<id>`，用于 Obsidian tweet embed。不要使用 HTML tweet widgets、网页脚本或第三方嵌入代码。
   - 如果输入或可见页面只有 `https://x.com/<handle>/article/<id>`，且无法确认对应 status URL，记录 `article_url`，并把 `embed_url` 设为 `unavailable`；不要把 article URL 猜成 status URL。
3. 按状态机检查页面是否可读：
   - 第一次打开后如果正文足够，继续 cook。
   - 第一次打开后若出现登录墙、限流、错误页、正文缺失、article 无法展开、或只有无法理解的媒体，允许同一 URL 最多一次 reload 或 reopen。
   - 第二次仍不可读时，立即 fail fast：写入可选 `capture.md` 记录 blocker，向用户说明原因，不生成最终 cooked note，不生成信息图，不换工具、不找替代来源、不继续绕过。
...
3. 生成信息图：
   ...
   - 如果 imagegen 不可用、生成失败、无法复制原图到 cache、或无法产出最终 `infographic.webp`，本次 cook 视为 blocked：保留 `capture.md` 和已产生的诊断材料，向用户说明 blocker，不写最终 cooked note。
...
article_url: <https://x.com/.../article/... if applicable>
...
<!-- 二选一：有 embed_url 时保留这一行，并删除 article fallback 行。 -->
![](https://twitter.com/<handle>/status/<id>)

<!-- 二选一：只有 article_url 且 embed_url unavailable 时保留这一行，并删除 tweet embed 行。 -->
原文链接：<article-or-canonical-url>
...
- `原文` 只放 tweet embed 或可打开 URL，不粘贴完整原文。
- `原文` 的 tweet embed 必须使用 Obsidian 支持的 Markdown 外部图片语法 `![](https://twitter.com/<handle>/status/<id>)`。不要使用 HTML tweet widgets、网页脚本或第三方嵌入代码，因为 Obsidian 笔记不应依赖脚本执行。
- 如果只有 X article URL 且无法确认对应 status URL，`原文` 只写可打开 article/canonical URL，`Source Manifest` 写 `embed URL: unavailable`；不要伪造 `https://twitter.com/<handle>/status/<id>`。
- `内容地图` 应覆盖作者的展开顺序、主要论点、例子、张力、结论和对用户有价值的启发。短 post 可以简化，但不要省略结构。
- `关键论点` 必须区分 `作者明确说法`、`Agent 推断` 和 `我的启发`；不要把 Agent 理解伪装成作者原话。
- `批判性点评` 是必选章节，站在第三者角度点评原文内容本身，而不是顺从作者观点。可以指出论证跳跃、证据不足、概念混用、适用边界、缺失反例、叙事诱导、和对用户实践可能造成的误导。点评必须基于可见页面 capture，不要根据记忆或外部事实扩写；不确定处写成问题或限制，不要伪装成已核验结论。
- `遗漏与不确定` 必须说明可见页面浏览器自动化 capture 的可见范围、thread 是否 best-effort、article 是否完整、媒体是否未能理解。
